...
describe("frontend ux stays on the backend load path", () => {
  it("does not statically import LiveKit from face-strip or loading UI", () => {
    ...
    const joinGate = readFileSync(path.join(src, "components/room/join-gate.tsx"), "utf8");
    ...
    expect(home).not.toMatch(/from\s+["'](ably|livekit-client)["']/);
    expect(joinGate).not.toMatch(/from\s+["'](ably|livekit-client)["']/);
    expect(joinGate).not.toMatch(/from\s+["']@\/lib\/client\/(realtime|livekit)["']/);
  });
});
...
describe("presence chrome", () => {
  it("lists who is here from the roster without extra work", () => {
    const presence = whoIsHere(
      [
    ...
    expect(presence.countLabel).toBe("2 here");
    expect(presence.namesLabel).toMatch(/Ada \(you\) · host/);
  ...
  it("surfaces mute and camera state on tiles", () => {
    expect(tileCamLabel({ camOn: false })).toBe("Cam off");
    expect(tileCamLabel({ camOn: false, camDenied: true })).toBe("Cam blocked");
    expect(tileCamLabel({ camOn: true })).toBeNull();
    expect(tileMicLabel(true)).toBe("Mic on");
    expect(tileMicLabel(false)).toBe("Mic off");
  });
});
